...
IF "%MSVC_VERSION%"=="2015" (
	cmake .. -G "Visual Studio 14 Win64" -DPSMOVE_USE_MADGWICK_AHRS=1 -DPSMOVE_USE_PS3EYE_DRIVER=1 -DPSMOVE_BUILD_OPENGL_EXAMPLES=1 -DPSMOVE_BUILD_TUIO_SERVER=ON -DOpenCV_DIR=./external/opencv/build/
	IF !ERRORLEVEL! NEQ 0 (
	...
	IF "%MSVC_VERSION%" == "2013" (
		cmake .. -G "Visual Studio 12 Win64" -DPSMOVE_USE_MADGWICK_AHRS=1 -DPSMOVE_USE_PS3EYE_DRIVER=1 -DPSMOVE_BUILD_OPENGL_EXAMPLES=1 -DPSMOVE_BUILD_TUIO_SERVER=ON -DOpenCV_DIR=./external/opencv/build/
		IF !ERRORLEVEL! NEQ 0 ( 
			echo Failed to generate PSMoveAPI solution
			goto Error
		)	
